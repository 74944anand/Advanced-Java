...
<body>
		...
		<nav aria-label="Page navigation">
			<ul class="pagination">
				<c:if test="${transactionsPage.totalPages > 1}">
					...
					<li class="page-item"><a class="page-link"
						href="?page=${transactionsPage.totalPages - 1}">Last</a>
					</li>
				</c:if>
			</ul>
		</nav>
	...
	<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('filterTransactionAmount').addEventListener('input', function() {
                filterTableByCriteria();
            });
            ...
            document.getElementById('dateFilterSubmit').addEventListener('click', function() {
                filterTableByCriteria();
            });
        ...
        function filterTableByCriteria() {
            ...
            var filterTransactionType = document.getElementById('filterTransactionType').value.toLowerCase();
            var filterTransactionAmount = parseFloat(document.getElementById('filterTransactionAmount').value);
            ...
            rows.forEach(function(row) {
                ...
                var transactionType = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
                var transactionAmount = parseFloat(row.querySelector('td:nth-child(2)').textContent);
                var transactionDate = row.querySelector('td:nth-child(3)').textContent.trim();
                var transactionDateObj = new Date(transactionDate);
                var startDateTime = new Date(startDate);
                var endDateTime = new Date(endDate);

                var validSearch = searchText === '' || transactionText.includes(searchText);
                var validType = filterTransactionType === '' || transactionType === filterTransactionType;
                var validAmount = isNaN(filterTransactionAmount) || transactionAmount === filterTransactionAmount;
                var validDateRange = (startDate === '' || transactionDateObj >= startDateTime) && (endDate === '' || transactionDateObj <= endDateTime);

                if (validSearch && validType && validAmount && validDateRange) {
                    row.style.display = '';
                } else {
